...
max_seq_length = 128
#classes = ['Neg', 'Pos']
classes = ['BUY', 'HOLD', 'NOT RATED', 'OUTPERFORM', 'TRADING BUY']


tokenizer = AutoTokenizer.from_pretrained("solikang/koelectra-small-v3-nsmc")
...
def model_fn(model_path=None):
    ...
    model = ElectraForSequenceClassification.from_pretrained('solikang/koelectra-small-v3-nsmc')   
    model.to(device)
    return model
...
def predict_fn(transformed_inputs, model):
    ...
    for data in transformed_inputs:
        ...
        print(f"sotfmax_output: {softmax_output}")
